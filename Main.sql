...
create view clients as (select distinct client from chats);

create view starts as (select * from chats where message = "@startVideo");
create view stops as (select * from chats where message = "@stopVideo");

select * from starts join stops
on starts.ha = stops.ha AND starts.client = stops.client;

select timestamp from chats
where message = "@stopVideo" AND 
